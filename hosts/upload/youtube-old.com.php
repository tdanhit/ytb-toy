...
if(substr($dVideoId, 0, 7) == 'miomio_')
{
	$videoId = str_replace('miomio_', '', $dVideoId);
	$dUrl = 'http://www.miomio.tv/watch/'.$videoId;
	$dPage = dat_curl($dUrl);
	if(stristr($dPage, 'class="showtitle"')) {
		$ttitle = get3Str('class="showtitle"', 'title="', '"', $dPage);
		$vdescription = $ttitle;
		$ttags = '';
		if(stristr($dPage, 'class="tagshow">'))
		{
			$dContent = get3Str('<ul', 'class="tagshow">', '</ul>', $dPage);
			$liContent = explode('<li', $dContent);
			for($cc = 1; $cc < count($liContent); $cc++)
			{
				$ttags .= get3Str('<a', 'nk">', '</', $liContent[$cc]) . ",";
			}
			$vtags = $ttags;
		}
	}
}
elseif(substr($lname, 0, 2) == 'sm')
{
		//if video exist
		$videoId = explode('.' , $lname);
		$videoId = str_replace('_render', '', $videoId[0]);
		$pageContent = @file_get_contents('http://ext.nicovideo.jp/api/getthumbinfo/'.$videoId);
		if(stristr($pageContent, '<title>')) $ttitle = str_replace('&', '&amp;', get2Str('<title>', '</title>', $pageContent));
		if(stristr($pageContent, '<description>')) $vdescription = str_replace('&', '&amp;', get2Str('<description>', '</description>', $pageContent));
		if(stristr($pageContent, '<tags'))
		{
			$ct = get2Str('<tags', '</tags', $pageContent);
			$ex = explode('<tag', $ct);
			$ttags = array();
			for($i = 1; $i < count($ex); $i++)
			{
				$ttags[] = str_replace('&', '&amp;', get2Str('>', '</', $ex[$i]));
			}
			if(count($ttags)) $vtags = implode(',', $ttags);
		}
		//echo $vtags;die();
		//die();
		$vcategory = 'Animals';
}elseif(substr($lname, 0, 7) == 'lights_')
{
		//if video exist
		$videoId = explode('lights_' , $lname);
		$videoId = $videoId[1];
		$videoId = explode('.', $videoId);
		$videoId = $videoId[0];

		$pageContent = @file_get_contents('http://www.lightsource.com/ministry/'.$videoId);
		$viewLink = get3Str('class="playVideo', 'href="', '"', $pageContent);
        $pageContent = @file_get_contents($viewLink);
		if(stristr($pageContent, 'property="og:title"')) $ttitle = str_replace('&', '&amp;', get3Str('property="og:title"', 'content="', '"', $pageContent));
		if(stristr($pageContent, 'name="description"')) $vdescription = str_replace('&', '&amp;', get3Str('name="description"', 'content="', '"', $pageContent));
		if(stristr($pageContent, 'name="keywords"'))
		{
			$vtags = get3Str('name="keywords"', 'content="', '"', $pageContent);
		}
		$vcategory = 'People';
} else {
...
//end get video info
	...
	if(@$_SESSION['videoTags'] != '') {
		if($_SESSION['tagsPosition'] == 1) $vtags = $_SESSION['videoTags'];
		else $vtags = $vtags . ',' . $_SESSION['videoTags'];
	}
	...
	if (preg_match('@<error>(.*)</error>@i', $upfiles, $err)) echo("Error: (".htmlentities($err[1] . ENT_QUOTES).") .");
	if (!preg_match('@<yt:videoid>([^<]+)</yt:videoid>@i', $upfiles, $vid)) echo("Error: Video ID not found.");
