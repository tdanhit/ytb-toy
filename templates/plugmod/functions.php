...
function renderActions() {
	...
	if (!$options['disable_chamthan']) $return .= '<option value="chamthan">Render Chấm Than</option>';
	if (!$options['disable_at1']) $return .= '<option value="at1">Render ÂT nhẹ + HA nhẹ</option>';
	if (!$options['disable_at2']) $return .= '<option value="at2">Render ÂT nhẹ + HA nặng</option>';
	if (!$options['disable_at3']) $return .= '<option value="at3">Render ÂT nặng + HA nhẹ</option>';
	if (!$options['disable_at4']) $return .= '<option value="at4">Render ÂT nặng + HA nặng</option>';
